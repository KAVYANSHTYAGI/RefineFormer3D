...
# --- NEW CUBE UTILITY ---
def compute_DHW(N):
    """ Compute (D, H, W) from sequence length N """
    n = round(N ** (1/3))
    if n**3 != N:
        D = H = W = n
        leftover = N - (n**3)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        D, H, W = x.shape[2:]  # Save D, H, W
        x = x.flatten(2).transpose(1, 2)  # (B, N, C)
        x = self.norm(x)
        return x, (D, H, W)  # Very important!! RETURN shape



# --- DeformableSelfAttention3D (major fix here) ---
class DeformableSelfAttention3D(nn.Module):
    ...
    def forward(self, x, D, H, W):  # takes D, H, W

        B, N, C = x.shape
        n = cube_root(N)  # assumes D = H = W
        ...
        offset = self.offset_conv(x_.transpose(1, 2).reshape(B, C, D, H, W))
        offset = offset.view(B, self.num_heads, 3, D, H, W)
        offset = torch.tanh(offset) * self.offset_range_factor

        # ⚡ Corrected grid resolution
        base = torch.linspace(-1, 1, steps=D, device=x.device)
        grid = torch.stack(torch.meshgrid(base, base, base, indexing='ij'), dim=0)
        grid = grid.unsqueeze(0).unsqueeze(0)

        sampling_grid = (grid + offset).permute(0, 1, 3, 4, 5, 2)
        sampling_grid = sampling_grid.reshape(B*self.num_heads, D, H, W, 3)

        x_kv = x_.transpose(1, 2).reshape(B, C, D, H, W)

        sampled_k = F.grid_sample(x_kv, sampling_grid, mode='bilinear', align_corners=True)
        sampled_v = F.grid_sample(x_kv, sampling_grid, mode='bilinear', align_corners=True)
...
class TransformerBlock3D(nn.Module):
    ...
    def forward(self, x, D, H, W):  # now takes x, D, H, W
        x = x + self.attn(self.norm1(x), D, H, W)  # Correct
